...
int validateUser(const char* username, const char* password) {
    ...
    /*
    Doctor doctor = getDoctor(0); // this should be by name
    
    if(-1 == doctor.doctorID) {
        // Doctor is not registrated
        return 0;
    }
    else {
        if(!strcmp(doctor.password, password)) {
            // Access granted
            return 1;
        } else {
            // Invalid password
    ...
    */
    ...
    for (int i = 0; i < num_users; i++) {
        if (strcmp(doctors[i].name, username) == 0 && strcmp(doctors[i].password, password) == 0) {
            free(doctors);
